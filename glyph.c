...
static struct font fallbacks[MAX_FALLBACKS];
static int num_fallbacks;

enum { VMOVE = 1, VLINE, VCURVE };
...
static bool is_valid_ptr(const uint8_t *p, size_t len) {
    if (font.is_valid && p >= font.data && p + len <= font.data + font.size)
        return true;
    for (int i = 0; i < MAX_FALLBACKS; i++) {
        if (fallbacks[i].is_valid && p >= fallbacks[i].data &&
            p + len <= fallbacks[i].data + fallbacks[i].size)
            return true;
    }
    return false;
}
...
static int setup(struct font *f, uint32_t fontstart) {
    uint32_t cmap, t;
    ...
    if (fontstart == 0 && f->size >= 12 && memcmp(data, "ttcf", 4) == 0) {
        uint32_t num_fonts = read_ulong(data + 8);
        if (num_fonts > 0) {
            uint32_t offset = read_ulong(data + 12);
            if (offset < f->size) {
                fontstart = offset;
            }
        }
    ...
    /* Check for CFF outlines (OTTO) */
    if (f->size >= fontstart + 4 && memcmp(data + fontstart, "OTTO", 4) == 0) {
        return -1;
    }
    ...
    for (i = 0; i < num_tables; ++i) {
        ...
        uint16_t platform = read_ushort(data + enc);
        uint16_t eid = read_ushort(data + enc + 2);

        /* prefer Microsoft Unicode Full (10) over BMP (1) over others */
        if (platform == STBTT_PLATFORM_ID_MICROSOFT) {
            if (eid == STBTT_MS_EID_UNICODE_FULL) {
                f->index_map = cmap + read_ulong(data + enc + 4);
                best_eid = eid;
                break; /* Found the best one */
            } else if (eid == STBTT_MS_EID_UNICODE_BMP && best_eid < STBTT_MS_EID_UNICODE_BMP) {
                f->index_map = cmap + read_ulong(data + enc + 4);
                best_eid = eid;
            }
        } else if (platform == STBTT_PLATFORM_ID_UNICODE && best_eid < 0) {
            f->index_map = cmap + read_ulong(data + enc + 4);
        }
    }
    if (f->index_map == 0)
...
static int find_index(const struct font *f, uint32_t codepoint) {
    ...
    if (format == 0) { /* apple byte encoding */
        ...
        if (codepoint >= first && codepoint < first + count)
            return read_ushort(data + index_map + 10 + (codepoint - first) * 2);
        return 0;
    } else if (format == 2) {
        ...
        if (offset == 0)
            return codepoint + read_short(idx + segcount * 4 + 2 + 2 * item);

        return read_ushort(data + offset + (codepoint - start) * 2 + index_map +
...
static int32_t glyph_offset(const struct font *f, int glyph_index) {
    uint32_t g1, g2;
    ...
    return g1 == g2 ? -1 : (int32_t)g1; /* if length is 0, return -1 */
}

static int close_shape(struct vertex *vertices, int n, int was_off,
                       int start_off, int32_t sx, int32_t sy, int32_t scx,
                       int32_t scy, int32_t cx, int32_t cy) {
    if (start_off) {
        if (was_off)
            vinit(&vertices[n++], VCURVE, (cx + scx) >> 1, (cy + scy) >> 1, cx,
                  cy);
...
static int glyph_shape(const struct font *f, int glyph_index,
    ...
    *pvertices = NULL;
    ...
    if (ncontours > 0) {
            ...
            if (next_move == i) {
                ...
                if (!(flags & 1)) { /* if it's a curve */
                    if (was_off) /* two off-curve control points in a row means
                        ...
                        vinit(&vertices[vcount++], VCURVE, (cx + x) >> 1,
                              (cy + y) >> 1, cx, cy);
                    cx = x;
                    cy = y;
                    was_off = 1;
        ...
        vcount = close_shape(vertices, vcount, was_off, start_off, sx, sy, scx,
                             scy, cx, cy);
    } else if (ncontours < 0) {
        /* Compound shapes */
        uint16_t flags = (1 << 5);
        ...
        while (flags & (1 << 5)) {
            ...
            /* Append vertices */
            tmp = malloc((vcount + comp_num_verts) * sizeof(struct vertex));
            if (tmp == NULL) {
                if (vertices)
                    free(vertices);
                if (comp_verts)
            ...
            if (vcount > 0)
                memcpy(tmp, vertices, vcount * sizeof(struct vertex));
...
static void *hheap_alloc(struct hheap *hh, size_t size) {
    if (hh->first_free) {
        ...
        return (char *)(hh->head) + sizeof(struct hheap_chunk) +
               size * hh->num_in_head_chunk;
    }
}
...
static float sized_trapezoid_area(float height, float top_width,
                                  float bottom_width) {
    assert(top_width >= 0);
...
static float position_trapezoid_area(float height, float tx0, float tx1,
                                     float bx0, float bx1) {
...
static void fill_active_edges_new(float *scanline, float *scanline_fill,
    ...
    while (e) {
        ...
        if (e->fdx == 0) {
            ...
            if (x0 < len) {
                if (x0 >= 0) {
                    handle_clipped_edge(scanline, (int)x0, e, x0, y_top, x0,
                                        y_bottom);
                    handle_clipped_edge(scanline_fill - 1, (int)x0 + 1, e, x0,
                                        y_top, x0, y_bottom);
                } else {
                    handle_clipped_edge(scanline_fill - 1, 0, e, x0, y_top, x0,
                                        y_bottom);
                }
            }
        } else {
            ...
            if (x_top >= 0 && x_bottom >= 0 && x_top < len && x_bottom < len) {
                ...
                if ((int)x_top == (int)x_bottom) {
                    ...
                    step =
                        sign * dy * 1; // dy is dy/dx, change in y for every 1
                                       // change in x, which is also how much
                                       // pixel area changes for each step in x
                    for (x = x1 + 1; x < x2; ++x) {
                        scanline[x] +=
                    ...
                    scanline[x2] +=
                        area + sign * position_trapezoid_area(
                ...
                for (x = 0; x < len; ++x) {
                    ...
                    if (x0 < x1 &&
                        ...
                        handle_clipped_edge(scanline, x, e, x1, y1, x3, y3);
                               ...
                               x3 >
                                   x1) { /* two segments across x, down-right */
                        handle_clipped_edge(scanline, x, e, x0, y0, x1, y1);
                        handle_clipped_edge(scanline, x, e, x1, y1, x3, y3);
                    } else if (x3 < x1 &&
                        ...
                        handle_clipped_edge(scanline, x, e, x0, y0, x1, y1);
                        handle_clipped_edge(scanline, x, e, x1, y1, x3, y3);
                    } else if (x0 < x2 && x3 > x2) { /* two segments across x+1,
                                                        down-right */
                        handle_clipped_edge(scanline, x, e, x0, y0, x2, y2);
                        handle_clipped_edge(scanline, x, e, x2, y2, x3, y3);
                    } else if (x3 < x2 && x0 > x2) { /* two segments across x+1,
                                                        down-left */
                        handle_clipped_edge(scanline, x, e, x0, y0, x2, y2);
                        handle_clipped_edge(scanline, x, e, x2, y2, x3, y3);
                    } else { /* one segment */
...
static void rasterize_sorted_edges(struct bitmap *result, struct edge *e, int n,
    ...
    while (j < result->h) {
        ...
        while (e->y0 <= scan_y_bottom) {
            if (e->y0 != e->y1) {
                struct active_edge *z = new_active(&hh, e, off_x, scan_y_top);
                if (z != NULL) {
                    if (j == 0 && off_y != 0) {
                        if (z->ey < scan_y_top) {
                            z->ey = scan_y_top;
    ...
    for (pass = 0; pass < 2; ++pass) {
        ...
        for (i = 0; i < num_verts; ++i) {
            ...
            case VLINE:
                ...
                break;
            case VCURVE:
                tesselate_curve(points, &num_points, x, y, vertices[i].cx,
...
static void render(struct bitmap *result, float flatness_in_pixels,
                   struct vertex *vertices, int num_verts, float scale_x,
                   float scale_y, float shift_x, float shift_y, int x_off,
                   int y_off, int invert) {
    float scale = scale_x > scale_y ? scale_y : scale_x;
    int winding_count, *winding_lengths;
    struct point *windings =
        flatten(vertices, num_verts, flatness_in_pixels / scale,
                &winding_lengths, &winding_count);
    if (windings) {
        rasterize(result, windings, winding_lengths, winding_count, scale_x,
                  scale_y, shift_x, shift_y, x_off, y_off, invert);
...
unsigned char *new_glyph(uint32_t id, uint32_t c, int cwidth) {
    ...
    if (glyph == 0) {
        for (int i = 0; i < num_fallbacks; i++) {
            int fb_glyph = find_index(&fallbacks[i], c);
            if (fb_glyph != 0) {
                f_ptr = &fallbacks[i];
                glyph = fb_glyph;
                break;
    ...
    struct bitmap bm = {font.width * cwidth, font.height, font.width * cwidth,
                        NULL};
    ...
    if (cwidth > 1) {
        /* Center double-width glyphs if they are narrower than 2 cells */
        float gw = get_advance(f_ptr, glyph) * f_ptr->scale;
        float bw = font.width * cwidth;
        if (gw < bw) shift_x += (bw - gw) / 2.0f;
    }

    render(&bm, 0.35f, vertices, vcount, draw_scale_x, draw_scale_y, shift_x,
           shift_y, xmin, ymin, 1);
...
int font_init(int size, char *path, int *w, int *h) {
    ...
    *h = font.height;
    ...
    /* Setup automatic fallback font integration (Nerd Font, CJK, etc.) */
    num_fallbacks = 0;
    const char *fb_paths[] = {
        ...
        "/run/current-system/sw/share/X11/fonts/ipaexg.ttf",
        "/run/current-system/sw/share/X11/fonts/ipag.ttf",
        "/run/current-system/sw/share/X11/fonts/vlgothic.ttf",
        "/usr/share/fonts/TTF/SymbolsNerdFont-Regular.ttf",
        "/usr/share/fonts/TTF/DroidSansFallback.ttf",
        NULL};

    for (int i = 0; fb_paths[i] && num_fallbacks < MAX_FALLBACKS; ++i) {
        struct font test_f;
        if (open_font_file(&test_f, fb_paths[i]) == 0) {
            uint32_t n_fonts = 1;
            if (test_f.size >= 12 && memcmp(test_f.data, "ttcf", 4) == 0) {
                n_fonts = (test_f.data[8] << 24) | (test_f.data[9] << 16) |
                          (test_f.data[10] << 8) | test_f.data[11];
            }
            close_font_file(&test_f);

            for (uint32_t j = 0; j < n_fonts && j < 3 && num_fallbacks < MAX_FALLBACKS; ++j) {
                struct font *f = &fallbacks[num_fallbacks];
                if (open_font_file(f, fb_paths[i]) == 0) {
                    uint32_t offset = 0;
                    if (n_fonts > 1) offset = read_ulong(f->data + 12 + j * 4);
                    
                    num_fallbacks++;
                    if (setup(f, offset) == 0) {
                        f->num_metrics = read_ushort(f->data + f->hhea + 34);
                        int fb_descent = get_descent(f);
                        int fb_ascent = get_ascent(f);
                        f->scale = (float)size / (fb_ascent - fb_descent);
                        f->ascent = floor(f->scale * fb_ascent);
                    } else {
                        num_fallbacks--;
                        close_font_file(f);
                    }
